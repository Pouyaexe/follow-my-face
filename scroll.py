...
def zoom_in(image, x, y, w, h, zoom_factor):
    ...
    # Check if the face is near to one of the bezels.
    is_near_bezel = (start_x <= 0 or end_x >= width or start_y <= 0 or end_y >= height)

    # If the face is not near to one of the bezels, keep the frame box centered on the face.
    if not is_near_bezel:
        start_x, start_y, end_x, end_y = face_x - face_width / 2 * zoom_factor, face_y - face_height / 2 * zoom_factor, face_x + face_width / 2 * zoom_factor, face_y + face_height / 2 * zoom_factor

    # Make sure the starting and ending x and y coordinates of the frame box are within the bounds of the image.
