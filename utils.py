...
def zoom_in(image: np.ndarray, x: float, y: float, w: float, h: float, zoom_factor: float) -> np.ndarray:
    ...
    face_x: float = x + face_width / 2
    face_y: float = y + face_height / 2

    # Calculate the starting and ending x and y coordinates of the face in the image.
    start_x: float = face_x - face_width / 2 * zoom_factor
    start_y: float = face_y - face_height / 2 * zoom_factor
    end_x: float = face_x + face_width / 2 * zoom_factor
    end_y: float = face_y + face_height / 2 * zoom_factor
    ...
    face_image: np.ndarray = image[int(start_y) : int(end_y), int(start_x) : int(end_x)]
